...
// Instance unique conservée en mémoire (Singleton)
...
export function getSupabaseClient(): SupabaseClient {
  ...
  // Réutilise la même instance pour éviter de dupliquer GoTrueClient
  if (!supabaseInstance) {
...
export async function ensureUserProfile(
  ...
  userId: string,
  profile: { email: string; phone?: string; full_name?: string; role?: string }
) {
  const { error } = await supabase.from("users").upsert(
    {
      id: userId,
...
export async function listPendingRides(supabase: SupabaseClient) {
  const { data, error } = await supabase
    .from("rides")
    .select("id, rider_id, pickup_address, dropoff_address, distance_km, price_cfa, created_at")
    .eq("status", "requested")
...
export async function acceptRide(supabase: SupabaseClient, rideId: string, driverId: string) {
  const { data, error } = await supabase
    .from("rides")
    .update({ status: "accepted", driver_id: driverId, updated_at: new Date().toISOString() })
    .eq("id", rideId)
